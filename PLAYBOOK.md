...
然后只需粘贴任意 YouTube 链接，并在输入时明确你的意图（例如“生成深度解读文章”或“整理为对话体逐字稿”）：

```
帮我将这个视频整理为对话体逐字稿：https://youtu.be/am_oeAoUhew
```

Claude 将会自动识别意图调用对应技能，并处理接下来的所有步骤（字幕抓取、生成排版、本地归档、Notion 上传）——无需提供额外的干预。
...
- **更改输出语言：** 编辑 `skills/article_generation.md` 或 `skills/dialogue_transcript.md` 中的「输出语言」规则。
- **更改文章结构：** 编辑 `skills/article_generation.md` 中的模板，或调整 `dialogue_transcript.md` 中的格式要求。
