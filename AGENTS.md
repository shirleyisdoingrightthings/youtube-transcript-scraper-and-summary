...
2. **区分"必改硬伤"与"可选润色"**：只有事实错误 / 逻辑反转 / 中英文术语对不上（如 JEPA 全称漏字）属于**必改硬伤**；语序、节奏、措辞属于**可选润色**，由用户定。
3. **术语前后一致**：对照 `glossary.md`（项目术语表）统一译法与写法（如"嵌入向量"不混用"嵌入表征"、"网络/编码器"段内统一、模型名连字符风格、"其他/其它"）。同一系列上下集之间也必须拉齐。
4. **每批给总判定**：明确输出 `✅ 本批无硬伤` 或 `⚠️ 本批 N 处硬伤`，不让用户自己猜。
5. **以"板块"为单位逐个核校**，便于用户逐段确认；发现硬伤立即修正后再进入 Step 3。
...
3. 向用户展示如下提示（不超过 3 行）：
   ...
   > 建议新增规则：「{规则草稿}」
   > 写入位置：`{AGENTS.md 或 skills/illustrated_deepdive.md / skills/dialogue_transcript.md 的具体段落}`，是否确认？
